...
func ChatHistoryListByRoom(room string, offset, limit int64) ([]ChatHistory, error) {
	var list []ChatHistory
	if err := DAL().Query(
		`SELECT * FROM "CHAT_HISTORY" WHERE "ROOM" = $1 ORDER BY "CREATE_TIME" ASC,"INDEX" ASC LIMIT $2 OFFSET $3`,
		&room, &limit, &offset,
	).List(&list); err != nil {
